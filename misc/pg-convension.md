...
* 命名必须以`putong-`开头，如果是分片库必须以`-shard`结尾。
* 多个部分使用`-`连接。例如：`putong-chat-shard`，`putong-payment`等，总共不超过三段。
...
* 所有数据库默认有三个基础角色： `putong-read`，`putong-write`，`putong-usage`，分别拥有所有表的只读，只写，函数的执行权限。
* 生产用户使用`putong-`作为前缀。生产用户，ETL用户，个人用户通过继承相应的基础角色获取权限。



==【强制】== **模式命名规则**

* 所有业务请使用`yay`作为模式名，无特殊理由不应在其他模式中创建对象。
...
* 存储过程适用于封装事务，减少并发冲突，减少RT，减少数据返回量，执行少量自定义逻辑。
...
* 反例：与数据获取、筛选密切关联的计算逻辑：如PostGIS中的几何关系判断。
...
* 建议主键使用统一的发号器函数，不建议直接使用`BIGSERIAL`。
...
* 能使用专有类型的，不使用字符串。（数值，网络地址，货币，JSON，UUID等）
...
- 索引页必须能放下至少三个字段才能发挥作用，单字段大小不得超过2KB。
- 可以对大字段取哈希，并建立函数索引。
...
- 默认隔离等级为**读已提交**，适合大多数简单读写事务。
- 需要事务级一致性快照的写事务，请使用**可重复读**隔离等级
- 对正确性有严格要求的写事务请使用**可序列化**隔离等级。
...
【强制】关注数据库与表的年龄，避免事物ID回卷。

【强制】关注表与索引的膨胀率，避免性能劣化。

【强制】监控复制延迟，使用复制槽时更必须十分留意。
